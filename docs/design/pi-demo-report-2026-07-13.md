...
A second fresh extension called `pi.sendUserMessage("Reply exactly INJECT-OK.")` immediately after `session_start` in an otherwise idle RPC session. Pi emitted an `input` event with source `extension`; the injected turn ran to `agent_settled`, then emitted `session_shutdown` when the controller closed stdin. The reply content was not captured in this probe. This directly validates the primary inbound-injection path. `sendMessage` and mid-stream steering/follow-up remain API-inventory findings pending dedicated probes. The probe source and event log are `/tmp/pi-characterization-20260713/injection-probe.ts` and `/tmp/pi-characterization-20260713/injection-probe/extension.jsonl`.
