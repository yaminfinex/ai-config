...
- Write the managed mise `conf.d` PATH file for the repo `bin/` directory and
  `tools/herder/shims/`.
...
PATH modification is managed by mise, not shell startup blocks. `ai-setup` writes
`${XDG_CONFIG_HOME:-$HOME/.config}/mise/conf.d/ai-config.toml` and refuses unmanaged files at that
path.
