...
<context>
This skill runs during planning or refactoring, when the shape of a module, API, or boundary is important enough to design deliberately. It is not for picking between two libraries. It is for shaping the interface your own code will expose.

Typical invocation points:
- A plan says "add module X" and the shape of X is not obvious.
- Two callers are about to grow coupled to an interface that has not been designed on purpose.
- A refactor surfaces a boundary that needs to be reconsidered deliberately.
</context>
...
If the environment supports parallel agents and the user has authorized delegation, run 3–4 design agents concurrently. Otherwise, generate the designs yourself. Each design gets the same requirements brief plus one divergent constraint:
...
- Capture the chosen shape, rejected alternatives, and reason in the relevant design doc, ADR, issue, plan, or conversation summary.
- Keep the interface sketch close enough to the implementation plan that future work can follow it without re-deriving the decision.
- Do not write the implementation unless the user explicitly asks to move from design to execution.
...
- [ ] The decision is captured in the relevant design doc, ADR, issue, plan, or conversation summary.
