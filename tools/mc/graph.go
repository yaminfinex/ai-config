...
func renderGraphSVG(model *graphModel, mission, focus, user string, query url.Values) template.HTML {
	...
	b.WriteString(`<style>.cluster{fill:var(--c-bg);stroke:var(--c-line)}.cluster-label{font:600 13px var(--font-prose);fill:var(--c-fg-dim)}.node rect{fill:var(--c-surface);stroke:var(--c-fg);stroke-width:1.5}.node text{font:13px var(--font-prose);fill:var(--c-fg)}.node .meta{font-size:11px;fill:var(--c-fg-dim)}.node .unread-text{fill:var(--c-surface)}.node.ghost{opacity:.42}.node.ghost rect{fill:none;stroke-dasharray:4 3}.node.desk rect{stroke:var(--c-needs-you);stroke-width:2}.edge{fill:none;stroke:var(--c-fg-dim);stroke-dasharray:6 5}.edge.managed{stroke:var(--c-needs-you);stroke-dasharray:none}.edge.raise{stroke:var(--c-warn)}.edge.cool2{opacity:.6}.edge.cool3{opacity:.3}.edge-label{font:11px var(--font-prose);fill:var(--c-fg-dim);paint-order:stroke;stroke:var(--c-surface);stroke-width:3px}.dim{opacity:.12}.legend{font:12px var(--font-prose);fill:var(--c-fg-dim)}a:focus .node rect{stroke-width:3}#arrow-dim path{fill:var(--c-fg-dim)}#arrow-accent path{fill:var(--c-needs-you)}#arrow-raise path{fill:var(--c-warn)}</style>`)
	b.WriteString(`<defs><marker id="arrow-dim" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto-start-reverse" markerUnits="strokeWidth"><path d="M0,0 L8,4 L0,8 z"/></marker><marker id="arrow-accent" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto-start-reverse" markerUnits="strokeWidth"><path d="M0,0 L8,4 L0,8 z"/></marker><marker id="arrow-raise" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto-start-reverse" markerUnits="strokeWidth"><path d="M0,0 L8,4 L0,8 z"/></marker></defs>`)
	...
	for _, node := range nodes {
		...
		if node.Kind == "desk" {
			...
			if node.Unread > 0 {
				fmt.Fprintf(&b, `<circle cx="%.0f" cy="%.0f" r="11" style="fill:var(--c-needs-you)"/><text class="unread-text" x="%.0f" y="%.0f" text-anchor="middle" font-size="10">%d</text>`, node.X+node.W-15, node.Y+15, node.X+node.W-15, node.Y+19, node.Unread)
			}
		}
		b.WriteString(`</g></a>`)
	}
