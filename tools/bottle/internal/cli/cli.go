...
var commands = []command{
	...
	stub("list", "bottle list",
		"Table of bottles: name, latest version, count, age, note"),
	stub("log", "bottle log <name>",
		"Version chain with provenance"),
	stub("show", "bottle show <name>[@v] [--turns N]",
		"Full metadata plus a transcript preview"),
	stub("rename", "bottle rename <old> <new>",
		"Move all versions to a new name"),
	stub("note", "bottle note <name>[@v] <text>",
		"Set or replace the free-text note"),
	stub("prune", "bottle prune",
		"Drop decant entries whose session files are gone"),
	stub("rm", "bottle rm <name>[@v]",
		"Delete one version, or the whole name"),
	stub("artifacts", "bottle artifacts <name>[@v] [--extract DIR]",
		"List or extract attached artifacts"),
}
