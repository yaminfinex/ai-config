...
// it. The order is load-bearing: resolve the ref, validate the run cwd BEFORE
// materializing (so a dead cwd never leaves an orphan seed), materialize the
// seed, record the decants-map entry (after a successful materialize, before
// launch), then build and exec the launch plan — interactively in this terminal
// or into a herdr split with --pane.
func cmdDecant(d *deps, args []string) int {
	...
	cwdOverride := fs.String("cwd", "", "run the decant in this cwd instead of the bottle's recorded one")
	...
	// gone cwd from leaving an orphan seed (U5 orphan-seed ordering rule).
	if fi, err := os.Stat(runCwd); err != nil || !fi.IsDir() {
		fmt.Fprintf(d.stderr, "bottle decant: recorded cwd %s no longer exists; pass --cwd to decant elsewhere\n", runCwd)
		return 1
	}
