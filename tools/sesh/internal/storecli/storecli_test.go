...
// testRoot builds a command tree the way ./cmd/sesh-store does: the store
// commands under a bare root. The client commands are irrelevant to these
// tests and would drag their config surface in.
func testRoot() *cobra.Command {
	root := &cobra.Command{Use: "sesh", SilenceUsage: true}
	root.AddCommand(Commands()...)
	return root
}

func TestCommandsCoverStoreSubcommands(t *testing.T) {
	var names []string
	for _, cmd := range Commands() {
		names = append(names, cmd.Name())
	}
	for _, want := range []string{"serve", "reindex", "admin"} {
		found := false
		...
		if !found {
			t.Errorf("Commands() missing %q (got %v)", want, names)
		}
	}
}
