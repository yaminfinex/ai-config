...
Pi ships only when `~/.pi/agent/sessions` is itself a real directory (a
symlinked root is rejected), and only regular, non-symlink files matching the
exact two-level shape shown above. The rest of `~/.pi/agent` (auth, settings,
models, extensions, and runtime state), non-matching session files, and deeper
paths never ship.
...
labeled degraded view rather than a 500. The branch graph is a single-flighted,
stale-while-revalidating in-memory projection keyed by mirrored membership,
generation, append time, and index counts. Its off-lock rebuild streams each
mirror generation once in canonical `(file_ordinal, line_ordinal)` append
order; warm page requests only slice and render their newest 200 projected
rows instead of reopening every graph row.
...
the legacy transcript routes' single-session lookups stay fully live; Pi
uses the same projection snapshot for its row counts and membership so a
bounded branch page does not first rescan the complete tree. Every
