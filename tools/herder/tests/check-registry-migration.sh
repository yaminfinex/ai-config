...
for test_name in "${test_names[@]}"; do
  if [[ -n "${declared_test_names[$test_name]+present}" ]]; then
    printf 'REGISTRY GATE DECLARATION DUPLICATED — "%s" appears more than once; remove the duplicate or replace it with the distinct test that anchors the missing invariant in check-registry-migration.sh.\n' "$test_name" >&2
    exit 1
  fi
...
if ((distinct_test_count < minimum_test_count)); then
  printf 'REGISTRY GATE DECLARATION FLOOR VIOLATED — only %d distinct tests remain, but at least %d invariant anchors are required; restore any removed declarations or replace them with the tests that now anchor those invariants in check-registry-migration.sh.\n' "$distinct_test_count" "$minimum_test_count" >&2
  exit 1
fi
...
require_declared_tests() {
  ...
  for test_name in "$@"; do
    if ! grep -Fxq "$test_name" <<<"$listed_tests"; then
      printf 'DECLARED REGISTRY GATE TEST MISSING — "%s" does not exist; fix its name, or replace it with the test that now anchors this invariant in check-registry-migration.sh.\n' "$test_name" >&2
      missing=1
    fi
  done
  return "$missing"
...
if test_executed_and_passed "$test_output" "$missing_test_probe"; then
  printf '\nREGISTRY V1 MIGRATION GATE SELF-CHECK FAILED — the absent test "%s" had RUN and PASS evidence; fix execution-evidence validation in check-registry-migration.sh.\n' "$missing_test_probe"
  exit 1
fi
...
if test_executed_and_passed "$skip_probe_output" "$missing_test_probe"; then
  printf '\nREGISTRY V1 MIGRATION GATE SELF-CHECK FAILED — skip-shaped output for "%s" was accepted without PASS evidence; restore PASS-evidence validation in check-registry-migration.sh.\n' "$missing_test_probe"
  exit 1
fi
...
for test_name in "${test_names[@]}"; do
  ...
  if ! grep -Fq -- "=== RUN   $test_name" <<<"$test_output"; then
    printf '\nDECLARED REGISTRY GATE TEST DID NOT RUN — "%s" lacks RUN output; fix its name, or replace it with the test that now anchors this invariant in check-registry-migration.sh.\n' "$test_name"
  else
    printf '\nDECLARED REGISTRY GATE TEST DID NOT PASS — "%s" may be skipped; un-skip it, or replace it with the test that now anchors this invariant in check-registry-migration.sh.\n' "$test_name"
  fi
  exit 1
done
