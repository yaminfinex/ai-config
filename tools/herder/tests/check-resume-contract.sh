...
case "${1:-} ${2:-}" in
  ...
  "tab create")
    ...
    printf '%s\n' "$*" >>"$PROBE/herdr_start_argv"
		...
		jq -n --arg ws "$ws" '{result:{tab:{tab_id:"tab_new", workspace_id:$ws}, root_pane:{pane_id:"p_resumed", terminal_id:"term_RESUMED", workspace_id:$ws, tab_id:"tab_new", cwd:"/mock/cwd"}}}';;
  "pane split")
    ...
    printf '%s\n' "$*" >>"$PROBE/herdr_start_argv"
		...
		jq -n '{result:{pane:{pane_id:"p_resumed", terminal_id:"term_RESUMED", workspace_id:"ws_self", tab_id:"tab_self", cwd:"/mock/cwd"}}}';;
  "pane run")
