...
JSONL
  ...
  if jq -s -e '[.[] | select(.event=="seated" or .event=="unseated")] | (.[-2].event=="seated" and .[-1].event=="unseated")' "$STATE/registry.jsonl" >/dev/null; then
    pass "T-2 child-first turnover pair"
  else
    fail_case "T-2 child-first turnover pair" "$(cat "$STATE/registry.jsonl")"
  fi
  assert_jq "T-2 child has cleared_from and new sid" 'select(.event=="seated" and .lineage.cleared_from=="guid-old" and .sids[-1].sid=="new-sid")' "$STATE/registry.jsonl"
  assert_jq "T-2 child bus identity reverified" 'select(.event=="seated" and .lineage.cleared_from=="guid-old" and .seat.hcom_name=="bus-alpha" and .seat.hcom_verified==true)' "$STATE/registry.jsonl"
...
JSONL
  ...
  assert_jq "T-2 first sid recognises same GUID" 'select(.guid=="guid-first" and .event=="seated" and .state=="seated" and .sids[-1].sid=="first-sid" and .observed_via=="observer sid enrichment")' "$STATE/registry.jsonl"
