...
unset MOCK_HCOM_ROWS
COMPACT_SEED_REGISTRY="$ROW_SELF_REPAIRED_VERIFIED"
MOCK_HCOM_ROWS='this-is-not-json'
scenario then_refuse_roster_unavailable midturn guid --dry-run "$STEER" --then "$CONT"
MOCK_HCOM_ROWS='[{"name":"me-bus","joined":true,"session_id":"sess-other","launch_context":{}}]'
scenario then_refuse_armed_sid_live_miss midturn guid --dry-run "$STEER" --then "$CONT"
unset MOCK_HCOM_ROWS
COMPACT_SEED_REGISTRY="$ROW_SELF"
