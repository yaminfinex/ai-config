...
tail -n1 "$REG_DIR/registry.jsonl" | jq -e '.kind=="session" and .guid=="guid-race" and .event=="retired" and .state=="retired" and .label=="race" and .close_result=="closed" and (.status|not) and (.seat|not)' >/dev/null \
  && ok "race cull: v2 retired row present" || bad "race cull: v2 retired row present" "latest=$(tail -n1 "$REG_DIR/registry.jsonl")"
