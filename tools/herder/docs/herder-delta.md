...
- **`herdr pane send-keys` has a small, undocumented key vocabulary.** The confirmed accepted set includes `Enter`, `esc`, `C-c`, and herdr-native key names such as `ctrl+u` and `backspace`; tmux-style names such as `C-u`, `Ctrl-u`, `^U`, `BSpace`, and `Escape` (capital E) are rejected as `invalid_key`. If a composer ends up with doubled or stray text, clear it with `herdr pane send-keys <pane> ctrl+u`; queued bus delivery resumes at the next boundary. Use `Enter` only when the visible text is a real message that should submit. `herdr pane send-keys -h` does not list the accepted set. Do not experiment by sending keys to a *running* peer agent — see "Driving peer agents safely" below.
...
2. **Don't send `esc` to a working peer agent.** For both codex and claude it doubles as **interrupt**. If you have confirmed the composer holds stray garbage, clear it with `herdr pane send-keys <pane> ctrl+u`; if the buffer holds a legitimate draft, leave it alone or submit it deliberately with `Enter`. Do not experiment by sending keys to a *running* peer agent.
