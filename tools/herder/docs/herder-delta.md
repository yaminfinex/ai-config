...
Historical upstream docs warned: *"ids can compact when tabs, panes, or workspaces are closed. do not treat them as durable ids."* That was pre-0.7.0 behavior. In current herdr, pane ids are stable within a server run unless a pane moves across workspaces, where pane and workspace ids re-key; same-workspace tab moves keep the pane id. A stale within-run pane id points at nothing, not at a different agent. Across a herdr restart or `update --handoff`, all ids reshuffle, so any captured pane/workspace/tab id is still not durable. The HERDER_GUID we inject is the durable handle the registry pivots on; herdr-side ids and the agent-reported session id are correlated to it.
...
- **`pane_id` is display-only.** Pane / workspace / tab ids are stable within a herdr server run except for cross-workspace pane moves, but they are not history-durable: a move can re-key the pane id, and a restart or `update --handoff` reshuffles all ids. Any id captured earlier may now point at nothing, or after restart at a different live pane. This is why the registry pivots on `terminal_id` for live resolution, not the stored `pane_id`: `herder send` and `herder wait` resolve a guid/label to the agent's *current* pane by `terminal_id` (and refuse if the terminal isn't live, rather than mis-send to a reshuffled pane id), `herder list` reports live status/pane by `terminal_id`, and `herder cull` re-verifies `terminal_id` before closing. Do the same in any ad-hoc send/close path — never trust a captured `pane_id`.
- **Cull-after-reshuffle race.** Before id-reshuffle handling was added, `herder cull --pane <id>` could close the wrong pane after a restart if `pane_id` had been reassigned. Still possible if you bypass `herder cull` and call `herdr pane close` directly with a stale id. Always go through `herder cull`.
...
- **Pane-id re-keying can hide a live agent from cull.** Moving a pane across workspaces re-keys it, so a registry row's `pane_id` may point at nothing while the agent is alive at a new id; after restart, the stale id may point at someone else. `herder cull` now re-resolves the target by its move-stable `terminal_id` across the whole live agent list and retargets the close, only declaring `already_gone` when the terminal isn't live anywhere.
- **`herdr pane send-keys` has a small, undocumented key vocabulary.** The confirmed accepted set includes `Enter`, `esc`, `C-c`, and herdr-native key names such as `ctrl+u` and `backspace`; tmux-style names such as `C-u`, `Ctrl-u`, `^U`, `BSpace`, and `Escape` (capital E) are rejected as `invalid_key`. If a composer ends up with doubled or stray text, clear it with `herdr pane send-keys <pane> ctrl+u`; queued bus delivery resumes at the next boundary. A queued bus message rendered on the input line is not garbage; do not clear it, because it self-delivers at the next turn boundary. Use `Enter` only when the visible text is a real message that should submit. `herdr pane send-keys -h` does not list the accepted set. Do not experiment by sending keys to a *running* peer agent — see "Driving peer agents safely" below.
...
2. **Don't send `esc` to a working peer agent.** For both codex and claude it doubles as **interrupt**. If you have confirmed the composer holds stray garbage, clear it with `herdr pane send-keys <pane> ctrl+u`; if the input line holds a queued bus message, leave it alone because it will self-deliver at the next turn boundary. If the buffer holds a legitimate draft, leave it alone or submit it deliberately with `Enter`. Do not experiment by sending keys to a *running* peer agent.
