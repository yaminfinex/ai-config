...
func TestBacklogEndpointPinsMalformedAndContainmentRefusals(t *testing.T) {
	...
	tests := []struct {
		...
		{"/api/backlog?root=" + url.QueryEscape(t.TempDir()) + "&path=x", http.StatusNotFound, "not found"},
		{"/api/backlog?root=relative%2Froot&path=x", http.StatusNotFound, "unknown root"},
