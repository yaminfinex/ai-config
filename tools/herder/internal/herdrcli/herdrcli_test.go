...
func TestParseProcessInfoHerdr08Wire(t *testing.T) {
	got, err := ParseProcessInfo([]byte(`{"result":{"process_info":{"pane_id":"p_1","foreground_process_group_id":42,"foreground_processes":[{"pid":43,"name":"bash","argv":["bash"],"cwd":"/work"}]}}}`))
	if err != nil {
		t.Fatal(err)
	}
	if got.ShellPID != 0 || got.ForegroundProcessGroupID != 42 || len(got.Processes) != 1 || got.Processes[0].Name != "bash" {
