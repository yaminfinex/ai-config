...
`bind_timeout` (nothing went on the wire inside spawn's window, so the initial prompt is persisted
...
an explicit value overrides the family default of 60000 for Claude and 300000 for other bus agents)
and `HERDER_SPAWN_VERIFY_MS` (receipt window, default 20000). Claude/bash publish
...
reached via the sidecar's owned-process correlation. A tagged roster display name and its base-name
database coordinate remain distinct: delivery records the display name, while merge-missing-only
launch-context repair uses the base name hcom emitted. Duplicate names and conflicting correlates
still fail closed. A slash-command prompt arrives as message TEXT, not a typed
...
shim at its invoked `hcom` path so shim-only installations still work from any cwd.
`HERDER_REAL_HCOM` is the explicit escape hatch when discovery is unsuitable: set it to an
executable path for the launch, and herder preserves that invoked path through bridge startup so
symlink-manager dispatch still works.
...
Managed Grok launches give the detached supervisor one lifetime lock per seat. After its binder
...
`base_name`. Cull and observer-confirmed unseat paths quiesce the journal and stop the entire bridge
process group. The observer backstop automatically reaps only a row-confirmed non-seated bridge
after an eligibility grace and zero-client recheck; a rowless legacy bridge is report-only and names
the manual `stop-bridge` recipe because process age alone cannot prove owner death.
