...
test('status chips toggle only when their text passes the CSS cap', () => {
  ...
  assert.equal(statusChipTruncates(`${'a'.repeat(20)}${'\u00a0'.repeat(10)}b`), true)
  assert.equal(statusChipTruncates(`\u00a0${'a'.repeat(26)}`), true)
  assert.equal(statusChipTruncates(`\f\r${'a'.repeat(26)}\t`), false)
