...
test('the badge renders brand glyphs for claude and codex and text fallback otherwise', async () => {
  const { readFileSync } = await import('node:fs')
  const presentation = readFileSync(new URL('../src/shared/presentation.tsx', import.meta.url), 'utf8')
  assert.match(presentation, /claude: <svg/)
  assert.match(presentation, /codex: <svg/)
  assert.match(presentation, /if \(!fallback\) return null/)
  assert.match(presentation, /className="tool-badge glyph" title=\{tool\} aria-label=\{tool\}/)
  assert.match(presentation, /className="tool-badge" title=\{tool\} aria-label=\{tool\}>\{fallback\}/)
})
