...
test('QuickOpen clears the query and starts on the first openable row on open or mode changes', () => {
  assert.match(source, /useEffect\(\(\) => \{\s*setQuery\(''\)[\s\S]*?setSelection\(open \? quickOpenInitialSelection\(initialRows, ''\) : null\)\s*if \(!open\) return[\s\S]*?\}, \[open, mode\]\)/)
})
...
test('QuickOpen resets the selection only on a real query edit, never when the debounce settles', () => {
  ...
  assert.match(source, /onChange=\{\(event\) => \{\s*setQuery\(event\.target\.value\)[\s\S]*?setSelection\(normalMode \? quickOpenInitialSelection\(/)
