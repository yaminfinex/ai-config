...
export function FileResults({ resolution, activeIndex = -1, onSelect, empty = 'No current matches.', limit = 100 }: {
  ...
  return <><RootOutcomes resolution={resolution} />
    {resolution.candidates.length === 0 ? <p className="file-results-empty">{empty}</p> : <div className="file-results" role="listbox" aria-label="Resolved files">
      {visible.map((candidate, index) => <button type="button" role="option" aria-selected={index === activeIndex} className={index === activeIndex ? 'active' : ''}
        key={`${candidate.root}\0${candidate.path}`} onMouseDown={(event) => event.preventDefault()} onClick={() => onSelect(candidate)}>
        <span className="file-result-path">{candidate.path}</span>
        <span className="root-tag" title={candidate.root}>{rootLabel(candidate.root)}</span>
        <span className={`tier-tag ${candidate.tier}`}>{candidate.tier}</span>
      </button>)}
