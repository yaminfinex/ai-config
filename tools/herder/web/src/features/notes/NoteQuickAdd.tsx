...
export function NoteQuickAdd({ group, label }: { group: string, label: string }) {
  ...
  return <span className="note-quick-add">
    ...
    {open && <span className="note-quick-add-popover">
      <textarea ref={inputRef} rows={3} data-notes-quick-input={group} aria-label={`New note for ${label}`} placeholder="Add a note…" onBlur={(event) => { if (!event.currentTarget.value.trim()) setOpen(false) }} onKeyDown={(event) => {
        if (event.key === 'Escape') { setOpen(false); event.preventDefault(); return }
        if (noteTextSubmitAction({ ...event, isComposing: event.nativeEvent.isComposing })) { save(); event.preventDefault() }
      }} />
