...
The SSE hello reported build identity `source:34932ad87a2262d3`. Process inspection showed the worktree binary `herder-34932ad8` on port 4493 and the owner binary `herder-2c6a3bb0` still listening on untouched port 4400.
...
| [file-root-identity-mismatch.png](file-root-identity-mismatch.png) | A deliberately altered persisted file root restores an honest identity-mismatch tombstone and issues zero file requests for the mismatched root. |
...
- A two-group layout was refreshed and restored with both live agent panels intact.
- A preview file was absent from `herder.web.layout.v2`; double-clicking pinned it and persisted `root`, `rootIdentity`, `path`, `viewMode`, and `preview: false`.
- Corrupting `root` while leaving `rootIdentity` unchanged produced `File root no longer matches` and no request for the mismatched file.
...
- Web: ESLint, TypeScript, 90 Node tests, production build, and embedded dist drift all pass.
- Production bundle: JS 922.62 kB / 254.76 kB gzip; CSS 168.01 kB / 17.56 kB gzip.
...
Healthy signals are one `/api/events` connection for the union of open consumers, continued agent transcript updates in every visible split, clean browser logs, and an SSE hello whose build identity matches the worktree server. A duplicate EventSource, an identity-mismatched resource request, a default Dockview skin color, or a restore exception is a failure signal; mitigation is to reset `herder.web.layout.v2` and roll back the client bundle while leaving the stateless server unchanged.
