...
Upstream hcom candidate from a resumed-session adoption incident: `hcom start --as <prior-name>` can mint a fresh placeholder identity even when the resumed transcript session id already owns a joined row; later hooks rebind the transcript-owned row and leave the placeholder as an inactive husk. Herder now avoids this path during adoption by resolving the source row's durable transcript session id before attempting a name reclaim, but raw/manual recovery can still create the husk. Ask: when the calling/resumed transcript session id already owns a row, `start --as` should reuse or explicitly rename that row, or refuse with the existing owner named; it should not create a second identity. This is hcom-owned because the herder hcom shim transparently forwards the command and cannot arbitrate hcom's roster mutation.
<!-- SECTION:NOTES:END -->
