...
FRESH-WORKTREE SPAWN STRANDING (2026-07-16, class-3 variant): every fresh git worktree is a NEW path with no mise trust record, so a spawn into a just-created worktree strands at mise's INTERACTIVE trust prompt during shell init — the launcher times out bind capture (name NOT captured), herdr detection reports lost, and the seat recovers only when someone answers the prompt in the pane (send-keys) after which the agent joins the bus late (documented late-join recovery applies). Remedy candidates: pre-trust at worktree creation (herdr worktree create runs mise trust for the new path), or MISE_TRUSTED_CONFIG_PATHS in the spawned pane's launch env.

JANITOR ASYMMETRY COROLLARY (peer observation, 2026-07-16, same day's cleanup): the janitor also FAILED to reap a genuinely DEAD row — a codex seat whose process died ~01:17 stayed fossilized at "listening" for 4+ hours until manually dropped (last real event #99074), while the live-but-heartbeat-starved @owner seat above WAS reaped. The staleness signal evidently tracks heartbeat traffic, not process liveness, so it fires on exactly the wrong side of both failure modes. Upstream hcom candidate (janitor should key on liveness evidence, or at least treat recent non-heartbeat activity as liveness) — add to the upstream-filing ledger batch.
