...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Deterministic hcom resolution: the bridge never records an argv0-dispatch shim target as hcom — canonicalization must not resolve through a symlink whose target basename is not hcom (mise-shim class); the recorded hcom-bin execs successfully from ANY cwd including non-project dirs, no HERDER_REAL_HCOM needed
- [x] #2 Binder identity-invocation env is allowlist-BUILT (not scrub-listed from os.Environ()); hostile/foreign tool signals (CLAUDE*/CLAUDECODE/HCOM_*) present at launch never alter binding and never reach hcom hook-install
- [x] #3 Regression tests pin both classes: (a) fake argv0-dispatch shim on PATH is resolved past to the real hcom, cwd-independent; (b) identity-env allowlist mutation test in the env class that exposed the original failure
- [x] #4 HERDER_REAL_HCOM override still honored and documented as the escape hatch
<!-- AC:END -->
...
CLOSED 2026-07-14: merged 7587d22 (single unit commit chain 50f2685 + fix round c58b8f6). Deterministic resolution: PATH walk prefers a later real hcom, keeps the first argv0-dispatch shim as last-resort INVOKED path; shared internal/hcombin.ResolveExecPath invariant applied in launch resolution AND OpenBinder (binder no longer EvalSymlinks through argv0 shims — seat hcom-bin + exec stay the shim). Identity env allowlist-built. Review: incumbent opus APPROVE (full argv0 survival trace) + grok calibration APPROVE (found the binder re-canonicalization P1 the incumbent initially missed — orchestrator-verified); fix round 1 closed both convergent P1s. Gates: worker announced batteries 58/58 at 50f2685 and final head c58b8f6; orchestrator independent re-gate 58/58 at c58b8f6; post-merge 58/58 on main. HERDER_REAL_HCOM stays honored as escape hatch; bogus-override silent fallthrough left as-is (reviewer-accepted). NOTE: fleet binaries pick this up on next herder rebuild/deploy; owner workaround needed until then.
<!-- SECTION:NOTES:END -->
